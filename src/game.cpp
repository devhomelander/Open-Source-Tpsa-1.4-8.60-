...
Thing* Game::internalGetThing(Player* player, const Position& pos, int32_t index, uint32_t spriteId, stackPosType_t type) const
{
	...
	if (pos.y & 0x40) {
		...
		int32_t subType;
		if (it.isFluidContainer() && index < static_cast<int32_t>(sizeof(reverseFluidMap) / sizeof(uint16_t))) {
...
void Game::internalGetPosition(Item* item, Position& pos, uint16_t& stackpos)
{
	pos.x = 0;
	pos.y = 0;
	pos.z = 0;
...
void Game::playerMoveThing(uint32_t playerId, const Position& fromPos,
                           uint16_t spriteId, uint16_t fromStackPos, const Position& toPos, uint16_t count)
{
...
void Game::playerMoveItemByPlayerID(uint32_t playerId, const Position& fromPos, uint16_t spriteId, uint16_t fromStackPos, const Position& toPos, uint16_t count)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
...
void Game::playerMoveItem(Player* player, const Position& fromPos,
                          uint16_t spriteId, uint16_t fromStackPos, const Position& toPos, uint16_t count, Item* item, Cylinder* toCylinder)
{
	if (!player->canDoAction()) {
		uint32_t delay = player->getNextActionTime();
	...
	if (item == nullptr) {
		...
		if (fromPos.x == 0xFFFF) {
			if (fromPos.y & 0x40) {
				...
				fromIndex = static_cast<uint16_t>(fromPos.y);
			}
		} else {
			fromIndex = fromStackPos;
		}
	...
	if (item->isHangable() && toCylinderTile->hasFlag(TILESTATE_SUPPORTS_HANGABLE)) {
		...
		if (!Position::areInRange<1, 1, 0>(playerPos, mapToPos)) {
			...
			uint16_t itemStackPos = fromStackPos;
	...
	if (toPos.x == 0xFFFF) {
		if (toPos.y & 0x40) {
			...
			toIndex = static_cast<uint16_t>(toPos.y);
		}
	}
...
void Game::addMoney(Cylinder* cylinder, uint64_t money, uint32_t flags /*= 0*/)
{
	...
	while (crystalCoins > 0) {
		const uint16_t count = std::min<uint32_t>(1000, crystalCoins);
	...
	uint16_t platinumCoins = money / 1000;
	if (platinumCoins != 0) {
...
void Game::playerUseItemEx(uint32_t playerId, const Position& fromPos, uint16_t fromStackPos, uint16_t fromSpriteId,
                           const Position& toPos, uint16_t toStackPos, uint16_t toSpriteId)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
...
void Game::playerUseItem(uint32_t playerId, const Position& pos, uint16_t stackPos,
                         uint16_t index, uint16_t spriteId)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
