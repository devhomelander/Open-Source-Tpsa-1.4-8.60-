...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_POKEGENDER: {
			uint16_t gender;
			if (!propStream.read<uint16_t>(gender)) {
				return ATTR_READ_ERROR;
			}

			setPokeGender(gender);
			break;
		}

		case ATTR_POKETRANSFORM: {
			uint16_t transform;
			if (!propStream.read<uint16_t>(transform)) {
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_POKELIFE: {
			uint16_t life;
			if (!propStream.read<uint16_t>(life)) {
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_POKENATURE: {
			uint16_t nature;
			if (!propStream.read<uint16_t>(nature)) {
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_POKEBALLDATE: {
			std::string date;
			if (!propStream.readString(date)) {
				return ATTR_READ_ERROR;
			}

			setPokeballDate(date);
			break;
		}

		case ATTR_POKEADDON: {
			uint16_t addon;
			if (!propStream.read<uint16_t>(addon)) {
				return ATTR_READ_ERROR;
			}
		...
		case ATTR_POKEMOVES: {
			uint16_t moves;
			if (!propStream.read<uint16_t>(moves)) {
				return ATTR_READ_ERROR;
			}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	const std::string& pkname = getPokeball();
	if (!pkname.empty()) {
		propWriteStream.write<uint8_t>(ATTR_POKEBALL);
		propWriteStream.writeString(pkname);
	}

	uint16_t gender = getPokeGender();
	if (gender != 0) {
		propWriteStream.write<uint8_t>(ATTR_POKEGENDER);
		propWriteStream.write<uint16_t>(gender);
	}

	uint16_t transform = getTransform();
	if (transform != 0) {
		propWriteStream.write<uint8_t>(ATTR_POKETRANSFORM);
		propWriteStream.write<uint16_t>(transform);
	}

	uint16_t life = getLife();
	if (life != 0) {
		propWriteStream.write<uint8_t>(ATTR_POKELIFE);
		propWriteStream.write<uint16_t>(life);
	}

	uint16_t nature = getNature();
	if (nature != 0) {
		propWriteStream.write<uint8_t>(ATTR_POKENATURE);
		propWriteStream.write<uint16_t>(nature);
	}

	uint8_t level = getLevel();
	if (level != 0) {
		propWriteStream.write<uint8_t>(ATTR_POKELEVEL);
		propWriteStream.write<uint8_t>(level);
	}

	const std::string& pkdate = getPokeballDate();
	if (!pkdate.empty()) {
		propWriteStream.write<uint8_t>(ATTR_POKEBALLDATE);
		propWriteStream.writeString(pkdate);
	}

	uint16_t addon = getPokeAddon();
	if (addon != 0) {
		propWriteStream.write<uint8_t>(ATTR_POKEADDON);
		propWriteStream.write<uint16_t>(addon);
	}

	uint16_t moves = getPokeMoves();
	if (moves != 0) {
		propWriteStream.write<uint8_t>(ATTR_POKEMOVES);
		propWriteStream.write<uint16_t>(moves);
	}
