...
class ItemAttributes
{
	public:
		...
		void setPokeball(const std::string& _a) {
			setStrAttr(ITEM_ATTRIBUTE_POKEBALL, _a);
		}
		const std::string& getPokeball() const {
			return getStrAttr(ITEM_ATTRIBUTE_POKEBALL);
		}

		void setPokeGender(uint8_t gender) {
			setIntAttr(ITEM_ATTRIBUTE_GENDER, gender);
		}
		uint8_t getPokeGender() const {
			return static_cast<uint8_t>(getIntAttr(ITEM_ATTRIBUTE_GENDER));
		}

		void setTransform(uint32_t transform) {
			setIntAttr(ITEM_ATTRIBUTE_POKETRANSFORM, transform);
		}

		uint32_t getTransform() const {
			return static_cast<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_POKETRANSFORM));
		}

		void setLife(uint32_t transform) {
			setIntAttr(ITEM_ATTRIBUTE_POKELIFE, transform);
		}

		uint32_t getLife() const {
			return static_cast<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_POKELIFE));
		}

		void setNature(uint32_t nature) {
			setIntAttr(ITEM_ATTRIBUTE_POKENATURE, nature);
		}

		uint32_t getNature() const {
			return static_cast<uint32_t>(getIntAttr(ITEM_ATTRIBUTE_POKENATURE));
		}

		void setLevel(uint8_t level) {
			setIntAttr(ITEM_ATTRIBUTE_POKELEVEL, level);
		}

		uint8_t getLevel() const {
			return static_cast<uint8_t>(getIntAttr(ITEM_ATTRIBUTE_POKELEVEL));
		}

		void setPokeballDate(const std::string& _date) {
			setStrAttr(ITEM_ATTRIBUTE_POKEDATE, _date);
		}
		const std::string& getPokeballDate() const {
			return getStrAttr(ITEM_ATTRIBUTE_POKEDATE);
		}

		void setPokeAddon(uint16_t pokeaddon) {
			setIntAttr(ITEM_ATTRIBUTE_POKEADDON, pokeaddon);
		}
		...
		void setPokeMoves(uint16_t valueMoves) {
			setIntAttr(ITEM_ATTRIBUTE_POKEMOVES, valueMoves);
		}
	...
	private:
		...
		const static uint32_t intAttributeTypes = ITEM_ATTRIBUTE_ACTIONID | ITEM_ATTRIBUTE_UNIQUEID | ITEM_ATTRIBUTE_DATE
			...
			| ITEM_ATTRIBUTE_POKEADDON | ITEM_ATTRIBUTE_POKEMOVES;
...
class Item : virtual public Thing
{
	public:
		...
		void setPokeball(std::string& _a) {
			setStrAttr(ITEM_ATTRIBUTE_POKEBALL, _a);
		}
		const std::string& getPokeball() const {
			return getStrAttr(ITEM_ATTRIBUTE_POKEBALL);
