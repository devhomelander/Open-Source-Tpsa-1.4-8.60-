...
void ScriptEnvironment::insertItem(uint32_t uid, Item* item)
{
	...
	if (!result.second) {
		std::cout << std::endl << __TIME__ << " Lua Script Error: Thing uid already taken.";
	}
}
...
void LuaScriptInterface::reportError(const char* function, const std::string& error_desc, lua_State* L /*= nullptr*/, bool stack_trace /*= false*/)
{
	...
	std::cout << std::endl << __TIME__ << " Lua Script Error: ";

	if (scriptInterface) {
		std::cout << __TIME__ << ' [' << scriptInterface->getInterfaceName() << "] " << std::endl;

		if (timerEvent) {
			std::cout << __TIME__ << " in a timer event called from: " << std::endl;
		}

		if (callbackId) {
			std::cout << __TIME__ << " in callback: " << scriptInterface->getFileById(callbackId) << std::endl;
		}

		std::cout << __TIME__ << " " <<  scriptInterface->getFileById(scriptId) << std::endl;
	}

	if (function) {
		std::cout << function << __TIME__ << " (). ";
	}

	if (L && stack_trace) {
		std::cout << __TIME__ << " " << getStackTrace(L, error_desc) << std::endl;
	} else {
		std::cout << __TIME__ << " " <<  error_desc << std::endl;
	}
}
...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(ITEM_ATTRIBUTE_POKEBALL)
	registerEnum(ITEM_ATTRIBUTE_GENDER)
	registerEnum(ITEM_ATTRIBUTE_POKETRANSFORM)
	registerEnum(ITEM_ATTRIBUTE_POKELIFE)
	registerEnum(ITEM_ATTRIBUTE_POKENATURE)
	registerEnum(ITEM_ATTRIBUTE_POKELEVEL)
	registerEnum(ITEM_ATTRIBUTE_POKEDATE)
	registerEnum(ITEM_ATTRIBUTE_POKEADDON)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE1)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE2)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE3)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE4)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE5)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE6)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE7)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE8)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE9)
	registerEnum(ITEM_ATTRIBUTE_POKEMOVE10)
...
int LuaScriptInterface::luaGameLoadMap(lua_State* L)
{
	...
	g_dispatcher.addTask(createTask([path]() {
		try {
			...
			std::cout << __TIME__ << " [Error - LuaScriptInterface::luaGameLoadMap] Failed to load map: "
				<< e.what() << std::endl;
		}
	}));
	return 0;
...
int LuaScriptInterface::luaPlayerAddItem(lua_State* L)
{
	...
	if (parameters >= 4) {
		...
		if (it.stackable) {
			itemCount = std::ceil(count / 1000.f);
		}
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (immunity == "physical") {
				...
				std::cout << __TIME__ << " [Warning - Monsters::loadMonster] Unknown immunity name " << immunity << " for monster: " << monsterType->name << std::endl;
				lua_pushnil(L);
			}
		}
	} else {
	...
	if (monsterType) {
		...
		if (spell) {
			...
			if (g_monsters.deserializeSpell(spell, sb, monsterType->name)) {
				...
				std::cout << __TIME__ << " " << monsterType->name << std::endl;
				std::cout << __TIME__ << " [Warning - Monsters::loadMonster] Cant load spell. " << spell->name << std::endl;
			}
		} else {
			lua_pushnil(L);
		}
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (race == "venom") {
				...
				std::cout << __TIME__ << " [Warning - Monsters::loadMonster] Unknown race type " << race << "." << std::endl;
				lua_pushnil(L);
				return 1;
			}
			pushBoolean(L, true);
	...
	if (loot) {
		if (isNumber(L, 2)) {
			...
			if (ids.first == Item::items.nameToItems.cend()) {
				std::cout << __TIME__ << " [Warning - Loot:setId] Unknown loot item \"" << name << "\". " << std::endl;
				pushBoolean(L, false);
				return 1;
			}

			if (std::next(ids.first) != ids.second) {
				std::cout << __TIME__ << " [Warning - Loot:setId] Non-unique loot item \"" << name << "\". " << std::endl;
				pushBoolean(L, false);
				return 1;
			}
...
int LuaScriptInterface::luaSpellCreate(lua_State* L)
{
	...
	if (lua_gettop(L) == 1) {
		std::cout << __TIME__ << " [Error - Spell::luaSpellCreate] There is no parameter set!" << std::endl;
		lua_pushnil(L);
		return 1;
	}
		...
		if (lua_gettop(L) == 1) {
			...
			if (group) {
				...
				if (group != SPELLGROUP_NONE) {
					...
					std::cout << __TIME__ << " [Warning - Spell::group] Unknown group: " << getString(L, 2) << std::endl;
					pushBoolean(L, false);
					return 1;
				}
				pushBoolean(L, true);
			...
			if (primaryGroup && secondaryGroup) {
				...
				if (primaryGroup != SPELLGROUP_NONE) {
					...
					std::cout << __TIME__ << " [Warning - Spell::group] Unknown primaryGroup: " << getString(L, 2) << std::endl;
					pushBoolean(L, false);
					return 1;
				}
				...
				if (secondaryGroup != SPELLGROUP_NONE) {
					...
					std::cout << __TIME__ << " [Warning - Spell::group] Unknown secondaryGroup: " << getString(L, 3) << std::endl;
					pushBoolean(L, false);
					return 1;
				}
				pushBoolean(L, true);
			} else {
				std::cout << __TIME__ << " [Warning - Spell::group] Unknown primaryGroup: " << getString(L, 2) << " or secondaryGroup: " << getString(L, 3) << std::endl;
				pushBoolean(L, false);
				return 1;
			}
		}
...
int LuaScriptInterface::luaCreatureEventType(lua_State* L)
{
	...
	if (creature) {
		...
		if (tmpStr == "login") {
			...
			std::cout << __TIME__ << " [Error - CreatureEvent::configureLuaEvent] Invalid type for creature event: " << typeName << std::endl;
			pushBoolean(L, false);
		}
...
int LuaScriptInterface::luaMoveEventType(lua_State* L)
{
	...
	if (moveevent) {
		...
		if (tmpStr == "stepin") {
			...
			std::cout << __TIME__ << " Error: [MoveEvent::configureMoveEvent] No valid event name " << typeName << std::endl;
			pushBoolean(L, false);
		}
		pushBoolean(L, true);
	} else {
...
int LuaScriptInterface::luaMoveEventSlot(lua_State* L)
{
	...
	if (moveevent->getEventType() == MOVE_EVENT_EQUIP || moveevent->getEventType() == MOVE_EVENT_DEEQUIP) {
		...
		if (slotName == "head") {
			...
			std::cout << __TIME__ << " [Warning - MoveEvent::configureMoveEvent] Unknown slot type: " << slotName << std::endl;
			pushBoolean(L, false);
			return 1;
		}
	}
...
int LuaScriptInterface::luaGlobalEventType(lua_State* L)
{
	...
	if (global) {
		...
		if (tmpStr == "startup") {
			...
			std::cout << __TIME__ << " [Error - CreatureEvent::configureLuaEvent] Invalid type for global event: " << typeName << std::endl;
			pushBoolean(L, false);
		}
		pushBoolean(L, true);
	} else {
...
int LuaScriptInterface::luaGlobalEventTime(lua_State* L)
{
	...
	if (globalevent) {
		...
		if (hour < 0 || hour > 23) {
			std::cout << __TIME__ << " [Error - GlobalEvent::configureEvent] Invalid hour \"" << timer << "\" for globalevent with name: " << globalevent->getName() << std::endl;
			pushBoolean(L, false);
			return 1;
		}
		...
		if (params.size() > 1) {
			min = params[1];
			if (min < 0 || min > 59) {
				std::cout << __TIME__ << " [Error - GlobalEvent::configureEvent] Invalid minute \"" << timer << "\" for globalevent with name: " << globalevent->getName() << std::endl;
				pushBoolean(L, false);
				return 1;
			}
			...
			if (params.size() > 2) {
				sec = params[2];
				if (sec < 0 || sec > 59) {
					std::cout << __TIME__ << " [Error - GlobalEvent::configureEvent] Invalid second \"" << timer << "\" for globalevent with name: " << globalevent->getName() << std::endl;
					pushBoolean(L, false);
					return 1;
				}
			}
...
int LuaScriptInterface::luaWeaponAction(lua_State* L)
{
	...
	if (weapon) {
		...
		if (tmpStr == "removecount") {
			...
			std::cout << __TIME__ << " Error: [Weapon::action] No valid action " << typeName << std::endl;
			pushBoolean(L, false);
		}
		pushBoolean(L, true);
	} else {
	...
	if (weapon) {
		if (!getNumber<CombatType_t>(L, 2)) {
			...
			if (tmpStrValue == "earth") {
				...
				std::cout << __TIME__ << " [Warning - weapon:element] Type \"" << element << "\" does not exist." << std::endl;
			}
		} else {
...
int LuaScriptInterface::luaWeaponAmmoType(lua_State* L)
{
	...
	if (weapon) {
		...
		if (type == "arrow") {
			...
			std::cout << __TIME__ << " [Warning - weapon:ammoType] Type \"" << type << "\" does not exist." << std::endl;
			lua_pushnil(L);
			return 1;
		}
		pushBoolean(L, true);
	...
	if (weapon) {
		...
		if (!getNumber<CombatType_t>(L, 3)) {
			...
			if (tmpStrValue == "earth") {
				...
				std::cout << __TIME__ << " [Warning - weapon:extraElement] Type \"" << element << "\" does not exist." << std::endl;
			}
		} else {
...
void LuaEnvironment::executeTimerEvent(uint32_t eventIndex)
{
	...
	if (reserveScriptEnv()) {
		...
		std::cout << __TIME__ << " [Error - LuaScriptInterface::executeTimerEvent] Call stack overflow" << std::endl;
	}
