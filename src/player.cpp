...
void Player::onCreatureAppear(Creature* creature, bool isLogin)
{
	...
	if (isLogin && creature == this) {
		...
		std::cout << ">> tpsa - " << name << " has logged in ( " << "Level :" << level << " ) Ip: " << convertIPToString(getIP()) << std::endl;
...
void Player::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		std::cout << ">> tpsa - " << getName() << " has logged out ( " << "Level :" << level << " ) Ip : " << convertIPToString(getIP()) << std::endl;
...
Item* Player::getCorpse(Creature* lastHitCreature, Creature* mostDamageCreature)
{
	...
	if (corpse && corpse->getContainer()) {
		std::ostringstream ss;
		if (lastHitCreature) {
			ss << "You recognize " << "Name : " << getNameDescription() << " \n " << "level :" << level <<
				"\n" << (getSex() == PLAYERSEX_FEMALE ? "She" : "He") << " was killed by Name: " << lastHitCreature->getNameDescription() <<
				" " << '.';
		}
		else {
			ss << "You recognize " << "Name : " << getNameDescription() << " \n " << "level :" << level  <<
		...
		for (int32_t slotIndex = CONST_SLOT_FIRST; slotIndex <= CONST_SLOT_LAST; ++slotIndex) {
			...
			if (inventoryItem) {
				if (Container* subContainer = inventoryItem->getContainer()) {
					...
					uint32_t remainder = (1000 - inventoryItem->getItemCount());
		...
		*destItem = nullptr;
		...
		while (i < containers.size()) {
			...
			for (Item* tmpItem : tmpContainer->getItemList()) {
				...
				//try find an already existing item to stack with
				if (tmpItem->equals(item) && tmpItem->getItemCount() < 1000) {
					index = n;
					*destItem = tmpItem;
					return tmpContainer;
				}
