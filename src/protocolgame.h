...
class ProtocolGame final : public Protocol
{
	...
	private:
		...
		//containers
		void sendAddContainerItem(uint16_t cid, const Item* item);
		void sendUpdateContainerItem(uint16_t cid, uint16_t slot, const Item* item);
		void sendRemoveContainerItem(uint16_t cid, uint16_t slot);

		void sendContainer(uint16_t cid, const Container* container, bool hasParent, uint16_t firstIndex);
		void sendCloseContainer(uint16_t cid);
