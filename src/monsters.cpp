...
bool Monsters::loadFromXml(bool reloading /*= false*/)
{
	...
	if (!result) {
		printXMLError("Error - Monsters::loadFromXml", "data/pokemon/pokemons.xml", result);
		return false;
	}

	loaded = true;

	for (auto monsterNode : doc.child("pokemons").children()) {
		std::string name = asLowerCaseString(monsterNode.attribute("name").as_string());
		std::string file = "data/pokemon/" + std::string(monsterNode.attribute("file").as_string());
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	pugi::xml_node monsterNode = doc.child("pokemon");
	if (!monsterNode) {
	...
	if ((attr = monsterNode.attribute("script"))) {
		...
		if (scriptInterface->loadFile("data/pokemon/scripts/" + script) == 0) {
			...
			std::cout << "[Warning - Pokemons::loadPokemons] Can not load script: " << script << std::endl;
