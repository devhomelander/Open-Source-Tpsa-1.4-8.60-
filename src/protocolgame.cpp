...
void ProtocolGame::parseUseItem(NetworkMessage& msg)
{
	...
	uint16_t stackpos = msg.getByte();
	uint16_t index = msg.getByte();
...
void ProtocolGame::parseUseItemEx(NetworkMessage& msg)
{
	...
	uint16_t fromStackPos = msg.getByte();
	Position toPos = msg.getPosition();
	uint16_t toSpriteId = msg.get<uint16_t>();
	uint16_t toStackPos = msg.getByte();
...
void ProtocolGame::sendContainer(uint16_t cid, const Container* container, bool hasParent, uint16_t firstIndex)
{
	NetworkMessage msg;
	msg.addByte(0x6E);
	...
	msg.addByte(std::min<uint32_t>(0xFFF, container->size()));

	uint32_t i = 0;
	const ItemDeque& itemList = container->getItemList();
	for (ItemDeque::const_iterator cit = itemList.begin() + firstIndex, end = itemList.end(); i < 0xFFF && cit != end; ++cit, ++i) {
		msg.addItem(*cit);
	}
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendCloseContainer(uint16_t cid)
{
	NetworkMessage msg;
	msg.addByte(0x6F);
	msg.addByte(cid);
...
void ProtocolGame::sendAddContainerItem(uint16_t cid, const Item* item)
{
	NetworkMessage msg;
	msg.addByte(0x70);
	msg.addByte(cid);
...
void ProtocolGame::sendUpdateContainerItem(uint16_t cid, uint16_t slot, const Item* item)
{
	NetworkMessage msg;
	msg.addByte(0x71);
	msg.addByte(cid);
...
void ProtocolGame::sendRemoveContainerItem(uint16_t cid, uint16_t slot)
{
	NetworkMessage msg;
	msg.addByte(0x72);
	msg.addByte(cid);
...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	msg.addByte(player->isAccessPlayer() ? 0xFFF : lightInfo.level);
